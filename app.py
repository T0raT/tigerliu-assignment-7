...
app = Flask(__name__)
app.secret_key = "1982591ff19r5"  # Replace with your own secret key, needed for session management
...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    plt.figure()
    plt.hist(slopes, bins=30, color='purple', alpha=0.7, label="Simulated Slopes")
    plt.xlabel("Slope")
...
def hypothesis_test():
    ...
    if test_type == ">":
        # Proportion of simulated stats greater than the observed statistic
        p_value = np.mean(simulated_stats > observed_stat)
    elif test_type == "<":
        # Proportion of simulated stats less than the observed statistic
        p_value = np.mean(simulated_stats < observed_stat)
    elif test_type == "!=":
        # Proportion of simulated stats more extreme than the hypothesized value (two-tailed)
        p_value = np.mean(np.abs(simulated_stats - hypothesized_value) >= np.abs(observed_stat - hypothesized_value))

    # TODO 11: If p_value is very small (e.g., <= 0.0001), set fun_message to a fun message
    fun_message = None
    if p_value <= 0.0001:
        fun_message = "Wow! Super rare result! 🌟"
...
def confidence_interval():
    ...
    t_value = stats.t.ppf(1 - (1 - confidence_level) / 2, df=S - 1)  # t critical value for two-tailed CI
    margin_of_error = t_value * (std_estimate / np.sqrt(S))  # Standard error times the t critical value
    ...
    plt.plot([ci_lower, ci_upper], [0, 0], color="blue", linewidth=3, label=f"{confidence_level*100:.1f}% Confidence Interval")
